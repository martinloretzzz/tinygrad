...
print(subprocess.run(["ld", "-lm", "-shared", "Add.o", "-o", "Add.so"]))

lib = ctypes.CDLL('./Add.so')

fxn = lib['_kernel']
fxn.argtypes = [ctypes.c_int64, ctypes.c_int64]
fxn.restype = ctypes.c_float

a = np.asarray([1.1,2.2,3.3,4.4], dtype=np.float32)
b = np.asarray([1.1,2.2,3.3,4.4], dtype=np.float32)
c = np.asarray([1.1,2.2,3.3,4.4], dtype=np.float32)
r = np.zeros((4), dtype=np.float32)

buf_addr = [buf.ctypes.data for buf in [r, a]]
print("bufs:", buf_addr)
